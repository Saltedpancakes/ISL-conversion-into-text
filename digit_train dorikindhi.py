...
if not cam.isOpened():
    ...
    # Open a file to store predictions
    output_file = open("predictions.txt", "a")  # Opens file in append mode
    ...
    while True:
        ...
        prediction_text = number_dict[np.argmax(pred)]
        
        # Print and write prediction to the file
        print(prediction_text)
        output_file.write(prediction_text + "\n")  # Append prediction to file

        # Display prediction on the screen
        cv2.putText(frame, prediction_text, (10, 50), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1)
    ...
    # Release the camera, close windows, and close the file
    cam.release()
    cv2.destroyAllWindows()
    output_file.close()  # Close the file after the loop ends
